...
if circFlag
  ...
  idxObs = ~isnan(Ytot);
  mYtot  = zeros(size(Ytot,1),1);
  for i =1:length(mYtot)
      tmp = Ytot(i,:);
      mYtot(i) = mean(tmp(idxObs(i,:)));
  end    
  idxObs = double(idxObs);
  sYYobs = idxObs*idxObs';
  sYYobs(sYYobs==0) = NaN;
  
  Ytot   = bsxfun(@minus, Ytot, mYtot);
  Ytot(~idxObs) = 0;
  Yshift = Ytot;
  
  lamK = Ytot*Yshift'/2;
  lamK = lamK ./ sYYobs;
  %lamK = soft_impute(lamK, hankelSize);
  indX = 1:yDim;
  for k=1:hankelSize
  ...
  for k=1:(2*hankelSize-1)
    ...
    lamK   = Ytot*Yshift';
    lamK = lamK ./ sYYobs;
    %lamK = soft_impute(lamK, hankelSize, [], [], [], false);
    indX   = 1:yDim;
