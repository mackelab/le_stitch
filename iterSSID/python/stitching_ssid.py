...
def run_exp_iterSSID(seq, seq_f, n, num_iter=100,pi_method='proper', plot_flag=True):
    ...
    params, SIGfp_new, SIGyy_new, lPSIGfp, lPSIGyy = \
...
def iterSSID(seq, n, num_iter=100, init=None, alpha=1.,pi_method='proper'):
    ...
    if len(seq['sub_pops']) == 1:
        idx_overlap = seq['sub_pops'][0]
    elif len(seq['sub_pops']) == 2:
        idx_overlap = np.intersect1d(seq['sub_pops'][0], seq['sub_pops'][1])
    else:
        raise Exception('more than two subpopulations not implemented yet')

    SIGfp, SIGyy = generateCovariancesFP(seq=seq, hS=n)

    lnPSIGfp = np.isnan(SIGfp)    # logical versions, 
    lnPSIGyy = np.isnan(SIGyy)    # readily usable 
    lPSIGfp = np.invert(lnPSIGfp) # for indexing
    lPSIGyy = np.invert(lnPSIGyy) #

    PSIGfp  = SIGfp[lPSIGfp] # observed part of Hankel cov matrix
    PSIGyy  = SIGyy[lPSIGyy] # observed part of cov matrix
    ...
    for t in range(1,num_iter+1):
        ...
        SIGfp_new[lPSIGfp] = PSIGfp # make sure we keep the observed part right
        SIGfp_new[lnPSIGfp] = alpha*SIGfp_new[lnPSIGfp] \
                            +(1-alpha)*SIGfp_old[lnPSIGfp]
        SIGyy_new[lPSIGyy] = PSIGyy # make sure we keep the observed part right
        SIGyy_new[lnPSIGyy] = alpha*SIGyy_new[lnPSIGyy] \
                            +(1-alpha)*SIGyy_old[lnPSIGyy]

        SIGyy_new = symmetrize(SIGyy_new)

        SIGfp_old = SIGfp_new # attention: shallow copy!
        SIGyy_old = SIGyy_new # attention: shallow copy!

    return params, SIGfp_new, SIGyy_new, lPSIGfp, lPSIGyy
...
def construct_hankel_cov(params):
    ...
    Pi = dlyap(params['A'], params['Q'])

    SIGyy = params['C'].dot(Pi.dot(params['C'].T)) + params['R']
